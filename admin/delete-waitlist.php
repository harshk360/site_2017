<?php
    ...
    if (empty($errors)) {
            ...
            $waitId = mysqli_real_escape_string($connect_site, stripslashes($waitId));

            $get_query = mysqli_query($connect_site,"SELECT * FROM waitlist WHERE id='$waitId'") or die("Can not query the TABLE! " . mysqli_error($connect_site));
            if($get_query->num_rows==1) {
                // PUT to participants
                $put_query = mysqli_query($connect_site, "UPDATE waitlist SET deleted=1 WHERE id='$waitId' ") or die("Unable to delete participant. " . mysqli_error($connect_site));
                $data['success'] = true;
                $data['message'] = 'Waitlist participant ' . $_POST['firstName'] . ' ' . $_POST['lastName'] . ' deleted.';
                ...
                $delWebMail->Subject = "SITE " . $SITE_YEAR . " Waitlist Participant Deleted";
                $delWebMail->Body = "Hello " . $WEBMASTER[0] . ",
                    <br><br>
                    Id " . $waitId . " was deleted from the waiting list by " . $_SERVER['PHP_AUTH_USER'] . ".
                    <br><br>" . 
                    json_encode($_POST) . "
                    <br><br><br> 
